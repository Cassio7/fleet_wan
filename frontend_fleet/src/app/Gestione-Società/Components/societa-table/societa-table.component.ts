import { AfterViewInit, Component, effect, EventEmitter, inject, Input, OnDestroy, Output, SimpleChanges, ViewChild } from '@angular/core';
...
private readonly destroy$: Subject<void> = new Subject<void>();
  ...
  constructor(
    ...
    effect(() => {
      const selectedSocieta = this.gestioneSocietaService.societaFilter() as string[];
      this.societaTableData.data = this.societa.filter((soci: Company) => selectedSocieta.includes(soci.name));
    });
  }
  ...
  deleteSocieta(company: Company){
    ...
    .subscribe((result) => {
      this.gestioneSocietaService.deleteCompany(companyId).pipe(takeUntil(this.destroy$))
      .subscribe({
        next: () => {
          if(result){
            this.societa = this.societa.filter(societa => societa.id != companyId);
            this.societaTableData.data = this.societa;
            this.societaChange.emit(this.societa);
            this.openSnackbar(`Società ${company.name} eliminata`);
          }else{
            this.openSnackbar(`Eliminazione società ${company.name} annullata`);
          }
        },
        error: error => console.error(`Errore nell'eliminazione della societaà con id ${companyId}: `, error)
      });
    });
  }
