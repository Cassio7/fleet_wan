...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, MatButtonModule, MatSnackBarModule, CantieriTableComponent, CantieriFiltersComponent],
...
export class HomeGestioneCantieriComponent implements OnInit, OnDestroy{
  ...
  ngOnInit(): void {
    ...
    this.gestioneCantieriService.getAllGroups().pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (groups: Group[]) => {
        this.groups = groups;
  ...
  createNewCantiere(): void {
    ...
    dialogRef.afterClosed().subscribe((result: {name: string, comune: string}) => {
      if (result !== undefined) {
        ...
        if(result.comune){
          this.gestioneCantieriService.getGroupIdByName(result.comune).pipe(takeUntil(this.destroy$))
          .subscribe({
  ...
  private createCantiere(newWorksiteData: {name: string, groupId?: number}){
    ...
    this.gestioneCantieriService.createCantiere(newWorksiteData).pipe(takeUntil(this.destroy$))
    .subscribe({
      next: (createdWorksiteData: {message: string, worksite: WorkSite}) => {
        console.log('createdWorksiteData: ', createdWorksiteData);
        console.log('createdWorksiteData.cantiere ', createdWorksiteData.worksite);
        this.openSnackbar(`Nuovo cantiere ${createdWorksiteData.worksite.name} creato`)
